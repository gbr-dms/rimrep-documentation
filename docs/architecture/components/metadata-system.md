...
- Metadata harvester. Metadata harvester is comprised of a series of Argo Workflows that need be triggered manually or run automatically to make the metadata harvest easier.
...
Here's a flowchart of the detailed steps in the metadata pipeline. (The detailed steps for metadata harvester is in the [metadata harvester flowchart](#metadata-harvester-flowchart))
...
flowchart TD
  ...
  classDef green fill:#bbf7d0,stroke:#16a34a;
    ...
    subgraph metadata_harvester["Metadata Harvester"]
        ...
        read_data_to_create[read-links-with-humancurated-content]
        create_github_ticket[create-github-dataset-ticket]
        read_data_to_update[read-links-with-github-issueno]
        compare_metadata{compared-metadata-for-change}
        regenerate_datapackagejson[regenerate-datapackagejson]
        submit_pr_auto[submit-datapackagejson-pr-rimrepcatalog]
        update_dataset_ticket[update-dataset-ticket-with-harvested-fields]
        notify_stakeholders[notify-DMS-metadata-personnel]
        generate_datapackagejson[generate-datapackagejson]
        human_intervention[manual-update/complete-dataset-ticket]:::red
        submit_pr_manual[submit-datapackagejson-pr-rimrepcatalog]
        trigger_manual([manual-trigger-first-harvest]):::red
        trigger_auto([auto-trigger-update-by-datawatcher-or-cron])
